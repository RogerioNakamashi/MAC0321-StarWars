...
public class Batalha {
	...
	private static void battle(String jedi, String sith) {
		...
		while (mestreJedi.getHP() > 0 && lordeSith.getHP() > 0 && fim == false) {
			sithAction = evento();
			jediAction = evento();
			...
			if(sithAction == "attack" && jediAction == "defense") {
				...
				if(mestreJedi.golpes[atkJ].getPrioridade() == 3) {
					...
					if(lordeSith.getHP()<=0) {
						fim = true;
					}
				}
			} else if(sithAction == "attack" && jediAction == "attack") {
				if(mestreJedi.golpes[atkJ].getPrioridade() == 2 && lordeSith.golpes[atkS].getPrioridade() == 3) {
					System.out.println("Mestre " + mestreJedi.getNome() + "usou " + mestreJedi.golpes[atkJ].getNome() + ".");
					System.out.println("Lorde " + lordeSith.getNomeSith() + "tomou " + mestreJedi.golpes[atkJ].getDano() + "de dano.");
					lordeSith.setHP(lordeSith.getHP() - mestreJedi.golpes[atkJ].getDano());
					if(lordeSith.getHP()<=0) {
						fim = true;
					...
					System.out.println("Lorde " + lordeSith.getNomeSith() + "usou " + lordeSith.golpes[atkS].getNome() + ".");
					System.out.println("Mestre " + mestreJedi.getNome() + "tomou " + lordeSith.golpes[atkS].getDano() + "de dano.");
					mestreJedi.setHP(mestreJedi.getHP() - lordeSith.golpes[atkS].getDano());
					if(mestreJedi.getHP()<=0) {
						fim = true;
				...
				} else if(mestreJedi.golpes[atkJ].getPrioridade() == 3 && lordeSith.golpes[atkS].getPrioridade() == 2) {
					System.out.println("Lorde " + lordeSith.getNomeSith() + "usou " + lordeSith.golpes[atkS].getNome() + ".");
					System.out.println("Mestre " + mestreJedi.getNome() + "tomou " + lordeSith.golpes[atkS].getDano() + "de dano.");
					mestreJedi.setHP(mestreJedi.getHP() - lordeSith.golpes[atkS].getDano());
					if(mestreJedi.getHP()<=0) {
				...
				System.out.println("Os dois usaram esquivar e nada acontece.");
			}
